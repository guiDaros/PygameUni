...
#         if engine.isBusy: # Garante que nada esteja falando antes de um novo prompt
#             engine.stop()
...
# Importa a função útil do nosso arquivo externo
from recursos.funcoes_uteis import desenhar_texto_multilinha # Caminho ajustado para 'recursos'
...
# Ícone do jogo (opcional: substitua por um .ico ou .png menor)
try:
    icone = pygame.image.load('recursos/imagens/nave.png') # Nome do arquivo alterado para 'nave.png'
    pygame.display.set_icon(icone)
except pygame.error:
    print("Aviso: Não foi possível carregar o ícone do jogo. Usando ícone padrão.")
...
PLAYER_NAME = ""
GAME_STATE = "INPUT_NOME" # Estados: INPUT_NOME, BOAS_VINDAS, JOGO, PAUSE, GAME_OVER
listening_for_game_over_command = False # Nova flag para o comando de voz na tela de Game Over
game_over_voice_feedback_message = "" # Mensagem de feedback para o comando de voz no Game Over

# Personagem (Nave Espacial)
try:
    IMAGEM_PERSONAGEM = pygame.image.load('recursos/imagens/player.png').convert_alpha() # Nome do arquivo alterado para 'player.png'
    IMAGEM_PERSONAGEM = pygame.transform.scale(IMAGEM_PERSONAGEM, (150, 150))
except pygame.error:
    print("Erro ao carregar imagem do personagem. Usando placeholder.")
    # Placeholder se a imagem não for encontrada
    IMAGEM_PERSONAGEM = pygame.Surface((140, 140), pygame.SRCALPHA)
    pygame.draw.polygon(IMAGEM_PERSONAGEM, AZUL, [(70, 0), (0, 140), (140, 140)])

personagem_rect = IMAGEM_PERSONAGEM.get_rect(center=(LARGURA_TELA // 2, ALTURA_TELA - 50))

# Fundo
try:
    FUNDO_ESTRELAS = pygame.image.load('recursos/imagens/background.png').convert() # Nome do arquivo alterado para 'background.png'
    FUNDO_ESTRELAS = pygame.transform.scale(FUNDO_ESTRELAS, (LARGURA_TELA, ALTURA_TELA))
except pygame.error:
    print("Erro ao carregar imagem de fundo. Usando fundo preto.")
    FUNDO_ESTRELAS = pygame.Surface((LARGURA_TELA, ALTURA_TELA))
...
# Obstáculos (Asteroides)
try:
    IMAGEM_ASTEROIDE = pygame.image.load('recursos/imagens/obstacle.png').convert_alpha() # Nome do arquivo alterado para 'obstacle.png'
except pygame.error:
    print("Erro ao carregar imagem do asteroide. Usando placeholder.")
    IMAGEM_ASTEROIDE = pygame.Surface((50, 50), pygame.SRCALPHA)
    pygame.draw.circle(IMAGEM_ASTEROIDE, CINZA_ESCURO, (25, 25), 25)
...
# O satélite irá se mover de forma randômica
try:
    IMAGEM_SATELITE = pygame.image.load('recursos/imagens/nave.png').convert_alpha() # Nome do arquivo alterado para 'nave.png'
    IMAGEM_SATELITE = pygame.transform.scale(IMAGEM_SATELITE, (60, 60))
except pygame.error:
    print("Erro ao carregar imagem do satélite. Usando placeholder.")
    IMAGEM_SATELITE = pygame.Surface((60, 60), pygame.SRCALPHA)
        ...
        try:
            with open(LOG_FILE, 'r') as f:
                ...
                for line in lines:
                    ...
                    # Garante que 'nome_jogador' exista, para compatibilidade com logs antigos
...
def tela_game_over():
    """Tela de fim de jogo, mostrando Game Over, os últimos scores e opções de voz."""
    global GAME_STATE, listening_for_game_over_command, game_over_voice_feedback_message, score
    TELA.fill(PRETO)
    ...
    if not last_scores:
        ...
        for i, log in enumerate(last_scores):
            # Exibe o nome do jogador junto com a pontuação
            log_texto = FONTE_PEQUENA.render(
    ...
    exit_button_y = ALTURA_TELA - button_height - 20 # 20px da parte inferior
    
    # Posição do botão Jogar Novamente
    restart_button_y = exit_button_y - button_height - button_spacing
    
    # Posição do botão Comando de Voz
    voice_button_y = restart_button_y - button_height - button_spacing
    
    # Posição da mensagem de feedback de voz
    voice_feedback_y = voice_button_y - 40 # 40px acima do botão de voz

    # Botão para Comando de Voz
    if desenhar_botao("Comando de Voz", LARGURA_TELA // 2 - button_width // 2, voice_button_y, button_width, button_height, AZUL, VERDE):
        listening_for_game_over_command = True
        game_over_voice_feedback_message = "Diga 'Jogar Novamente' ou 'Sair do Jogo'."
        if engine.isBusy(): # Garante que nada esteja falando antes de um novo prompt
            engine.stop()
        engine.say("Diga 'Jogar Novamente' ou 'Sair do Jogo'.")
        engine.runAndWait()
    ...
    if desenhar_botao("Jogar Novamente", LARGURA_TELA // 2 - button_width // 2, restart_button_y, button_width, button_height, VERDE, AZUL):
        GAME_STATE = "INPUT_NOME" # Volta para a tela de input de nome
        PLAYER_NAME = "" # Limpa o nome para que a tela de input apareça novamente
        score = 0 # Reseta a pontuação
        game_over_voice_feedback_message = "" # Limpa feedback de voz
    ...
    # Exibe feedback de voz
    if game_over_voice_feedback_message:
        feedback_render = FONTE_PEQUENA.render(game_over_voice_feedback_message, True, BRANCO)
        feedback_rect = feedback_render.get_rect(center=(LARGURA_TELA // 2, voice_feedback_y))
        TELA.blit(feedback_render, feedback_rect)
...
while running:
    ...
    # --- Lógica de Atualização por Estado ---
    if GAME_STATE == "GAME_OVER" and listening_for_game_over_command:
        try:
            r = sr.Recognizer()
            with sr.Microphone() as source:
                print("Ouvindo comando de Game Over...")
                audio = r.listen(source, timeout=5, phrase_time_limit=3)
            
            command = r.recognize_google(audio, language='pt-BR').lower()
            game_over_voice_feedback_message = f"Comando: '{command}'"
            print(f"Comando de voz reconhecido: {command}")

            if "jogar novamente" in command or "reiniciar" in command:
                game_over_voice_feedback_message = "Iniciando nova partida!"
                if engine.isBusy(): engine.stop()
                engine.say("Iniciando nova partida!")
                engine.runAndWait()
                GAME_STATE = "INPUT_NOME"
                PLAYER_NAME = ""
                score = 0
            elif "sair do jogo" in command or "sair" in command:
                game_over_voice_feedback_message = "Saindo do jogo..."
                if engine.isBusy(): engine.stop()
                engine.say("Saindo do jogo.")
                engine.runAndWait()
                running = False # Encerra o loop principal
            else:
                game_over_voice_feedback_message = "Comando não reconhecido. Tente novamente."
                if engine.isBusy(): engine.stop()
                engine.say("Comando não reconhecido. Por favor, tente novamente.")
                engine.runAndWait()
            
            listening_for_game_over_command = False # Desativa a escuta após processar
        except sr.UnknownValueError:
            print("Não foi possível entender o áudio na tela de Game Over.")
            game_over_voice_feedback_message = "Não entendi. Tente novamente."
            if engine.isBusy(): engine.stop()
            engine.say("Não consegui entender. Por favor, tente novamente.")
            engine.runAndWait()
            listening_for_game_over_command = False
        except sr.WaitTimeoutError:
            print("Tempo de espera excedido para reconhecimento de voz (Game Over).")
            game_over_voice_feedback_message = "Nenhum áudio detectado. Clique novamente para tentar."
            if engine.isBusy(): engine.stop()
            engine.say("Tempo esgotado. Nenhuma voz detectada.")
            engine.runAndWait()
            listening_for_game_over_command = False
        except sr.RequestError as e:
            print(f"Erro no serviço de reconhecimento de voz (Game Over); {e}")
            game_over_voice_feedback_message = f"Erro de serviço: {e}"
            if engine.isBusy(): engine.stop()
            engine.say("Erro no serviço de reconhecimento de voz. Verifique sua conexão.")
            engine.runAndWait()
            listening_for_game_over_command = False
        except Exception as e:
            print(f"Ocorreu um erro inesperado no reconhecimento de voz (Game Over): {e}")
            game_over_voice_feedback_message = f"Erro inesperado: {e}"
            if engine.isBusy(): engine.stop()
            engine.say("Ocorreu um erro inesperado no reconhecimento de voz.")
            engine.runAndWait()
            listening_for_game_over_command = False
